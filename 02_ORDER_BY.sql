-- ORDER BY
-- select 문과 함께 사용하며 결과 집합을 특정 얼마나 값에 따라
-- 정렬하는데 사용된다.
...
from
    tbl_menu
order by
#     menu_price asc -- 내림차순 정렬
#     menu_price desc -- 내림차순 정렬
    menu_price; -- 오름차순 정렬(deafault)
...
-- 첫번쨰 인자가 두번째 인자 이후의 값들과 비교해서 일치하면
-- 해당 위치의 값을 보여준다.
*/
select field('A', 'B', 'C','A');
select field('B', 'A', 'B','C');

-- 데이터베이스에 들어있는 데이터 비교 할 수 있다.
...
order by
    ...
    ref_category_code desc; -- ref_category_code 내림차순으로 정렬
